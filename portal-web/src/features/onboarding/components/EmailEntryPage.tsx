...
export function EmailEntryPage() {
  const { t: tOnboarding } = useTranslation('onboarding')
  const { t: tCommon } = useTranslation('common')
  const navigate = useNavigate()

  const { plan } = useLoaderData({ from: '/onboarding/email' })
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      setOauthError(err instanceof Error ? err : new Error(String(err)))
    }
  }
              ...
              <form.FormRoot className="space-y-6">
                ...
                {displayedError && (
                  <div className="alert alert-error">
                    <span className="text-sm">{displayedError.message}</span>
                  </div>
                )}
