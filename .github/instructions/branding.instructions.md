...
<form className="space-y-4">
  ...
  <PasswordInput label="Password" required error={errors.password?.message} />
...
All icons must be sourced from **lucide-react** to ensure consistency, accessibility, and RTL compatibility.
...
### 12.2. Standard Icon Usage by Context

#### **Actions (CRUD Operations)**

| Action           | Icon     | Usage                                                     | Example                            |
| ---------------- | -------- | --------------------------------------------------------- | ---------------------------------- |
| **Create/Add**   | `Plus`   | Adding new items (orders, products, customers, addresses) | `<Plus size={20} />` Add Order     |
| **Edit/Modify**  | `Edit`   | Editing existing records                                  | `<Edit size={18} />` Edit Customer |
| **Delete**       | `Trash2` | Deleting records (customers, addresses, items)            | `<Trash2 size={18} />` Delete      |
| **View/Show**    | `Eye`    | Viewing details                                           | `<Eye size={18} />` View Details   |
| **Save/Confirm** | `Check`  | Confirmation, completed states                            | `<Check size={20} />` Saved        |
| **Close/Cancel** | `X`      | Closing modals, dismissing items, canceling               | `<X size={20} />` Close            |
| **Search**       | `Search` | Search inputs and functionality                           | `<Search size={20} />` Search      |
| **Filter**       | `Filter` | Filtering data                                            | `<Filter size={20} />` Filter      |

#### **Navigation**

| Context               | Icon            | Usage                            | RTL Handling                                                            |
| --------------------- | --------------- | -------------------------------- | ----------------------------------------------------------------------- |
| **Back**              | `ArrowLeft`     | Going back to previous page      | ⚠️ **Must rotate 180° in RTL**: `className={isRTL ? "rotate-180" : ""}` |
| **Next/Forward**      | `ChevronRight`  | Next step, forward navigation    | ⚠️ **Auto-flips in RTL with `start`/`end`**                             |
| **Previous**          | `ChevronLeft`   | Previous step                    | ⚠️ **Auto-flips in RTL with `start`/`end`**                             |
| **Dropdown Expand**   | `ChevronDown`   | Expanding dropdowns, menus       | No rotation needed                                                      |
| **Dropdown Collapse** | `ChevronUp`     | Collapsing dropdowns, menus      | No rotation needed                                                      |
| **Go to First Page**  | `ChevronsLeft`  | Pagination first page            | ⚠️ **Auto-flips in RTL**                                                |
| **Go to Last Page**   | `ChevronsRight` | Pagination last page             | ⚠️ **Auto-flips in RTL**                                                |
| **Menu Toggle**       | `Menu`          | Opening sidebar, navigation menu | No rotation needed                                                      |

#### **Resources & Entities**

| Entity               | Icon           | Usage                                          | Example          |
| -------------------- | -------------- | ---------------------------------------------- | ---------------- |
| **Business**         | `Building2`    | Business/workspace selector, business settings | Business Profile |
| **Customer/s**       | `Users`        | Customer list, customer section                | Customers Page   |
| **Single Customer**  | `User`         | Individual customer profile                    | Customer Detail  |
| **Product**          | `Package`      | Product/inventory items, product empty states  | Inventory        |
| **Variant**          | `Package`      | Product variants (same as product)             | Product Variants |
| **Order**            | `ShoppingCart` | Orders, shopping functionality                 | Orders Page      |
| **Orders (People)**  | `ShoppingBag`  | Customer order history count                   | Customer card    |
| **Team/Users**       | `UsersRound`   | Team management, multiple users                | Team Settings    |
| **Address/Location** | `MapPin`       | Addresses, location data                       | Customer Address |

#### **Communication & Contact**

| Context                   | Icon                | Usage                                | Example             |
| ------------------------- | ------------------- | ------------------------------------ | ------------------- |
| **Email**                 | `Mail`              | Email inputs, contact email          | Email Field         |
| **Phone**                 | `Phone`             | Phone numbers, contact information   | Phone Number        |
| **Website/Social**        | `Globe`             | Website URLs, online presence        | Website Link        |
| **Language**              | `Languages`         | Language switcher (alternative icon) | Language Menu       |
| **Language (Globe)**      | `Globe`             | Language switcher (primary icon)     | Language Selector   |
| **Social Media Platform** | Custom social icons | See `components/icons/social.tsx`    | Instagram, Facebook |
| **WhatsApp**              | `MessageCircle`     | WhatsApp contact (lucide fallback)   | WhatsApp Link       |

#### **Financial & Analytics**

| Context               | Icon              | Usage                              | Example                |
| --------------------- | ----------------- | ---------------------------------- | ---------------------- |
| **Money/Price**       | `DollarSign`      | Prices, total spent, revenue       | Total Revenue          |
| **Accounting**        | `Calculator`      | Accounting section                 | Accounting Page        |
| **Billing/Payment**   | `CreditCard`      | Billing, subscriptions, payments   | Billing Settings       |
| **Analytics**         | `BarChart3`       | Analytics dashboard                | Analytics Page         |
| **Dashboard**         | `LayoutDashboard` | Dashboard/home page                | Dashboard              |
| **Trending Up**       | `TrendingUp`      | Positive growth, increase          | Revenue Up 15%         |
| **Trending Down**     | `TrendingDown`    | Decline, decrease                  | Orders Down 5%         |
| **Charts (Line)**     | `LineChart`       | Line chart representations         | Line Chart Empty State |
| **Charts (Pie)**      | `PieChart`        | Pie/doughnut chart representations | Pie Chart Empty State  |
| **Charts (Activity)** | `Activity`        | Mixed charts, activity charts      | Activity Chart         |

#### **File Management**

| File Type           | Icon                     | Usage                        | Example            |
| ------------------- | ------------------------ | ---------------------------- | ------------------ |
| **Image**           | `ImageIcon` (as `Image`) | Image upload, image inputs   | Upload Photo       |
| **Camera**          | `Camera`                 | Camera capture, photo upload | Take Photo         |
| **Cloud Upload**    | `CloudUpload`            | File upload zones            | Drop files here    |
| **Generic File**    | `File`                   | Unknown file types           | Generic attachment |
| **Document**        | `FileText`               | PDF, text files, documents   | Document.pdf       |
| **Video**           | `Video`                  | Video files                  | Video.mp4          |
| **Audio**           | `Music`                  | Audio files                  | Audio.mp3          |
| **Archive**         | `Archive`                | ZIP, RAR, compressed files   | Archive.zip        |
| **Image Not Found** | `ImageOff`               | Missing image placeholder    | No Photo Available |

#### **States & Feedback**

| State             | Icon                          | Usage                             | Example                                                                |
| ----------------- | ----------------------------- | --------------------------------- | ---------------------------------------------------------------------- |
| **Loading**       | `Loader2`                     | Loading states, async operations  | ⚠️ **Must use `animate-spin`**: `<Loader2 className="animate-spin" />` |
| **Success**       | `CheckCircle`, `CheckCircle2` | Success messages, completed tasks | Payment Successful                                                     |
| **Error/Alert**   | `AlertCircle`                 | Error messages, warnings          | Error loading data                                                     |
| **Warning**       | `AlertTriangle`               | Warning states, caution           | Critical Error                                                         |
| **Info/Help**     | `HelpCircle`                  | Help text, information tooltips   | Need Help?                                                             |
| **Refresh/Retry** | `RefreshCcw`                  | Retry actions, refresh data       | Retry                                                                  |
| **Rotate**        | `RotateCw`                    | Rotating images, refreshing       | Rotate Image                                                           |

#### **Form & Input Elements**

| Context               | Icon           | Usage                            | Example         |
| --------------------- | -------------- | -------------------------------- | --------------- |
| **Password Show**     | `Eye`          | Show password toggle             | Show Password   |
| **Password Hide**     | `EyeOff`       | Hide password toggle             | Hide Password   |
| **Password Lock**     | `Lock`         | Password field indicator         | Password Input  |
| **Calendar**          | `Calendar`     | Date picker inputs               | Select Date     |
| **Time/Clock**        | `Clock`        | Time picker inputs               | Select Time     |
| **Checkbox Selected** | `Check`        | Selected state in custom selects | Selected Option |
| **Clear/Reset**       | `X`            | Clear input, reset form          | Clear Search    |
| **Drag Handle**       | `GripVertical` | Reorderable items, drag handle   | Reorder Items   |

#### **Miscellaneous**

| Context                | Icon           | Usage                         | Example             |
| ---------------------- | -------------- | ----------------------------- | ------------------- |
| **Settings**           | `Settings`     | Settings, configuration       | Settings Page       |
| **Logout**             | `LogOut`       | Sign out, logout action       | Sign Out            |
| **Under Construction** | `Construction` | Work in progress, coming soon | Feature coming soon |
| **Documentation**      | `BookOpen`     | Documentation, help articles  | View Docs           |

### 12.3. Icon Sizing Standards

**Standard Sizes:**

- **Inline with text (buttons):** `size={18}` or `size={20}`
- **Large interactive elements:** `size={24}`
- **Headers/titles:** `size={28}` to `size={32}`
- **Empty states:** `size={48}` to `size={64}`
- **Touch targets (IconButton):** `size={20}` (within 44px container)
...
### 12.4. Icon Accessibility Rules

**1. Decorative Icons (with text):**
...
```tsx
<IconButton icon={Trash2} aria-label="Delete customer" onClick={handleDelete} />
```

**3. Icons in Status/Informational Context:**
...
### 12.5. RTL Icon Handling

**Icons That Must Rotate in RTL:**

- `ArrowLeft` → Used for "Back" (must rotate 180°)
- `ArrowRight` → Used for "Forward"
...
**Icons That Auto-Flip (Chevrons in Pagination/Navigation):**

- `ChevronLeft`, `ChevronRight`, `ChevronsLeft`, `ChevronsRight`
- These automatically flip when used within logical properties (`start`/`end`)

**Icons That Never Rotate:**

- `ChevronDown`, `ChevronUp` (vertical)
- `Check`, `X`, `Plus`, `Trash2` (symmetrical)
- `Package`, `Users`, `Mail` (entity representations)

### 12.6. Icon Color & State

**Default State:**

```tsx
<Icon size={20} className="text-base-content/50" />
```
...
<button className="group">
  <Icon className="text-base-content/50 group-hover:text-primary transition-colors" />
</button>
```
...
```tsx
<Icon className="text-base-content/30" />
```
...
### 12.7. Custom Icon Components

For platform-specific icons (social media), use custom components in `components/icons/`:
...
<SocialIcon platform="whatsapp" size={20} /> // Falls back to MessageCircle
```
...
- **v2.1** (2026-01-02): Added comprehensive icon system documentation

  - Complete icon usage guide by context (actions, navigation, entities, etc.)
  - Standard icon sizing guidelines
  - RTL icon handling patterns
  - Icon accessibility rules
  - Color and state specifications
  - Custom icon component documentation
